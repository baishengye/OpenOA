...
- **EntryAbility 一行接入**：`import { initHotfix } from '@itc/hotfix'`，在 `onCreate`（`super.onCreate` 之前）调 `initHotfix()`。把第三方要求的 `AppStorage.setOrCreate('CodePushConfig', …)`（key 名 HAR 锁死）封装进库，对外只见中性的 `HotfixConfig`。
...
| **鸿蒙** | `harmony/entry/build-profile.json5` → `buildOption.arkOptions.buildProfileFields.Production`（及 `Staging`） | 同处 `ServerUrl` | 运行时由 [EntryAbility.onCreate](../apps/oa/harmony/entry/src/main/ets/entryability/EntryAbility.ets) 的 `initHotfix()` 读出写进 `AppStorage`；也可直接 `initHotfix({ production, serverUrl })` 传参 |
