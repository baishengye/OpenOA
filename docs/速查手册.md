...
> 一页纸版。详细从零搭建见 [从零搭建-*.md](./从零搭建-鸿蒙.md)，运行细节见 [运行-*.md](./运行-鸿蒙.md)，工具盘符位置见 [环境与磁盘布局.md](./环境与磁盘布局.md)。
> 本机：node 在 `/usr/local/bin/node`；工程根 `/Volumes/MacExtend/Code/ReactNative/OpenOA`，App 在 `apps/oa`。

---

## 2. 环境变量

### 2.1 已写进 `~/.zshrc`（持久，开新终端自动生效）
```bash
# Android SDK
...
open ios/OpenOA.xcworkspace            # Xcode 选模拟器/真机 Run（注意 .xcworkspace）
# 模拟器 CLI（可选）：
#  xcodebuild -workspace ios/OpenOA.xcworkspace -scheme OpenOA -configuration Debug \
#    -sdk iphonesimulator -destination "id=<UDID>" -derivedDataPath /tmp/dd build CODE_SIGNING_ALLOWED=NO
#  xcrun simctl install <UDID> /tmp/dd/Build/Products/Debug-iphonesimulator/OpenOA.app
#  xcrun simctl launch <UDID> org.reactjs.native.example.OpenOA
```
